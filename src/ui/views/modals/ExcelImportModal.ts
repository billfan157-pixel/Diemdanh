...
export class ExcelImportModal {
  ...
  private importResult: ExcelImportResult | null = null
  ...
  open(classId: string | null, onComplete?: () => void): void {
    if (classId) {
      ...
      if (!cls.students.length) {
        this.notificationManager.show('Lớp chưa có học viên. Import có danh sách sẽ tự động thêm.', 'info')
      }
    }
  ...
  private renderFilePicker(): void {
    ...
    btnNext.addEventListener('click', () => this.renderSheetPicker())
  }

  private async handleFile(file: File, fileDrop: HTMLElement, fileInput: HTMLInputElement, btnNext: HTMLButtonElement): Promise<void> {
  ...
  private renderSheetPicker(): void {
    ...
    if (sheetNames.length === 1) {
      ...
      if (!this.classId) {
        this.parseStudentListAndCreateClass()
      } else {
  ...
  private parseStudentListAndCreateClass(): void {
    const body = this.overlay?.querySelector('#modalBody')
    if (!body) return
    const rows = this.sheets[this.currentSheet]
    if (!rows || rows.length < 2) {
  ...
  private renderMultiClassForm(result: StudentListResult, suggestedYear: string): void {
    const body = this.overlay?.querySelector('#modalBody')
    if (!body) return
    ...
    body.querySelector('#btnBack')?.addEventListener('click', () => this.renderFilePicker())
    body.querySelector('#btnCreateAll')?.addEventListener('click', async () => {
      ...
      const { ensureScoresMatchColumns } = await import('../../../config/columns.ts')
      ...
      for (const clsName of result.detectedClasses) {
        ...
        const cols = this.stateManager.getClassColumns(newId)
        const newStudents = studentsInClass.map(row => ({
          id: generateId('st'),
          ...
          email: '',
          ghiChu: row.ghiChu || '',
          scoresByTerm: { hk1: ensureScoresMatchColumns(undefined, cols), hk2: ensureScoresMatchColumns(undefined, cols) },
          learningLog: [],
          createdAt: Date.now(),
          updatedAt: Date.now()
        }))
        this.stateManager.setClassStudents(newId, newStudents)
        created++
      }
      this.close()
  ...
  private renderSingleClassForm(result: StudentListResult, suggestedYear: string): void {
    const body = this.overlay?.querySelector('#modalBody')
    if (!body) return
    ...
    body.querySelector('#btnBack')?.addEventListener('click', () => this.renderFilePicker())
    body.querySelector('#btnCreateClass')?.addEventListener('click', async () => {
      ...
      const newStudents = result.rows.map(row => ({
        id: generateId('st'),
        tenThanh: row.tenThanh || '',
        hoDem: row.hoDem || '',
        ten: row.ten || '',
  ...
  private downloadTemplate(): void {
    ...
    const cols = cls.columns?.length ? cls.columns : (() => {
  ...
  private renderPreview(result: ExcelImportResult, term: 'hk1' | 'hk2'): void {
    const body = this.overlay?.querySelector('#modalBody')
    if (!body) return
    const cls = this.stateManager.getClass(this.classId!)
    if (!cls) return
    ...
    const matchedKeys = new Set(result.matchedCols.map(m => m.mappedTo))
    const displayCols = classCols.filter(c => matchedKeys.has(c.key))
    const headerLabels = ['Tên thánh', 'Họ đệm', 'Tên', ...displayCols.map(c => c.label), 'Ghi chú']
      ...
      <div class="step">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${previewRows.map(row => {
                const scores = displayCols.map(c => {
  ...
  private async executeImport(result: ExcelImportResult, term: 'hk1' | 'hk2', overwrite: boolean): Promise<void> {
    let targetClassId = this.classId

    // Auto-create class if no classId was provided
    if (!targetClassId) {
      const sheetName = this.currentSheet || 'Lớp mới'
      const suggestedName = sheetName.replace(/[_-]/g, ' ').trim()
      const state = this.stateManager.getState()
      const year = state.yearFilter || new Date().getFullYear().toString()

      // Ask user for class name
      const className = await this.promptClassName(suggestedName)
      if (!className) return // user cancelled

      targetClassId = this.stateManager.createClass(className, year)

      // Add the imported students as new students
      const classCols = resolveClassColumns(this.stateManager.getClass(targetClassId)!)
      const { ensureScoresMatchColumns } = await import('../../../config/columns.ts')
      const { generateId } = await import('../../../utils/id.ts')
      ...
      this.stateManager.setClassStudents(targetClassId, newStudents)
    } else {
      // Existing class — apply normally
      const cls = this.stateManager.getClass(targetClassId)
      if (!cls) return
      ...
      this.stateManager.setClassStudents(targetClassId, updatedStudents)
    }

    this.close()
    this.notificationManager.show(
      `✅ Import thành công — ${result.validRows} học viên${!this.classId ? ', lớp đã được tạo tự động' : ''} (${term === 'hk1' ? 'HK1' : 'HK2'})`,
      'success'
    )
    this.onComplete?.()
  }

  private promptClassName(suggested: string): Promise<string | null> {
    return new Promise((resolve) => {
      const body = this.overlay?.querySelector('#modalBody')
      if (!body) { resolve(null); return }

      // Change dialog content directly
      body.innerHTML = `
        <div class="step">
          <div style="text-align:center;margin-bottom:12px">
            <div style="font-size:2rem">📚</div>
            <strong>Tạo lớp mới từ file Excel</strong>
            <p class="hint">File không khớp với lớp nào. Nhập tên cho lớp mới:</p>
          </div>
          <div style="margin:16px 0">
            <label style="display:block;margin-bottom:6px;font-weight:600">Tên lớp</label>
            <input type="text" id="newClassNameInput" class="input"
              value="${suggested}"
              style="width:100%;padding:10px;border:1px solid var(--border);border-radius:6px;font-size:1rem"
              placeholder="VD: Ấu Nhi 1A" />
          </div>
          <p class="hint">Lớp sẽ được tạo với các cột điểm phát hiện từ file Excel (${this.importResult?.matchedCols.length || 0} cột).</p>
        </div>
        <div class="actions">
          <button class="btn btn-secondary" id="btnCancelCreate">Hủy</button>
          <button class="btn btn-primary" id="btnConfirmCreate">✅ Tạo lớp & Import</button>
        </div>
      `

      body.querySelector('#btnCancelCreate')?.addEventListener('click', () => resolve(null))
      body.querySelector('#btnConfirmCreate')?.addEventListener('click', () => {
        const input = body.querySelector('#newClassNameInput') as HTMLInputElement
