...
export function renderCardsView(props: CardsViewProps): string {
  ...
  const cards = students.map((st, i) => {
    ...
    const colsHtml = cols.map(col => {
      ...
      return `<div class="se-cell${empty ? ' is-empty' : ''}" data-col-key="${col.key}">
        ...
        <div class="add-score">
          ...
          <button type="button" data-add-score data-sid="${st.id}" data-col="${col.key}" title="Thêm" aria-label="Thêm">+</button>
        </div>
      </div>`
    }).join('')
    ...
    return `<article class="student score-card se-row" data-id="${st.id}">
      <div class="se-main">
        <div class="se-who">
          <div class="se-identity">
            <input type="checkbox" class="student-select" data-select-student="${st.id}" title="Chọn học viên" aria-label="Chọn ${escapeHtml(displayName(st))}" />
        ...
        <div class="student-actions se-actions">
          <button type="button" class="btn-icon btn-icon-neutral" data-move-student="${st.id}" title="Chuyển lớp">⇄</button>
          <button type="button" class="btn-icon btn-icon-neutral" data-journal="${st.id}" title="Nhật ký học vụ">📓</button>
          <button type="button" class="btn-icon" data-del-student="${st.id}" title="Xóa">×</button>
        </div>
      </div>
